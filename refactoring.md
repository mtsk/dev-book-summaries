...
* **Improving design without changing behavior** – restructure code to make it cleaner and easier to evolve while keeping functionality the same.
* **Continuous refactoring** – treat refactoring as a regular, ongoing activity, not a one-time cleanup.
* **Code smells as signals** – recognize common warning signs in code that indicate deeper design problems.
* **Small, safe steps** – apply tiny, controlled changes to minimize risk and make progress visible.
* **Refactoring catalog** – use named, proven refactorings as a shared vocabulary and practical toolbox.
* **Tests as a safety net** – rely on automated tests to refactor confidently and detect regressions early.
* **Emergent design** – allow better design and abstractions to evolve through incremental improvements.
